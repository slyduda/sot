...
// Add checks to make sure correct parameters are transfered
test("check to see if history works", () => {
  ...
  expect(objectMachine.history.length).toBe(1);
});

// Add checks to make sure correct parameters are transfered
test("check to see if history to works", () => {
  const myObject = new ExampleObject(START_ENERGY);
  const objectMachine = soter(myObject, exampleMachineDict);
  objectMachine.to("walking");
  expect(objectMachine.history.length).toBe(1);
});
